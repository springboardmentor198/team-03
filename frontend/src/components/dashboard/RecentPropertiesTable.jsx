    ...
    return (
      <div className="rounded-2xl border border-gray-100 dark:border-[#30363d] bg-white dark:bg-[#161b22] p-10 text-center">
        ...
        <p className="text-sm font-semibold text-gray-900 dark:text-[#e6edf3]">
          {t("recentTable.error.title")}
        </p>
        <p className="mt-1 text-xs text-gray-500 dark:text-[#7d8590]">
          {t("recentTable.error.description")}
        </p>
      </div>
    );
  }
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-[#30363d] bg-white dark:bg-[#161b22] shadow-sm">
      ...
      {properties.length === 0 ? (
        <div className="p-8 text-center">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-[#e6edf3]">
            {t("recentTable.empty.title")}
          </p>
          <p className="mt-1 text-sm text-gray-500 dark:text-[#7d8590]">
            {t("recentTable.empty.description")}
          </p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50/50 dark:bg-[#1c2128] text-xs text-gray-500 dark:text-[#7d8590]">
              <tr>
                <th className="px-6 py-4 font-medium">{t("recentTable.columns.address")}</th>
                <th className="px-6 py-4 font-medium">{t("recentTable.columns.type")}</th>
                <th className="px-6 py-4 font-medium">{t("recentTable.columns.status")}</th>
                <th className="px-6 py-4 font-medium">{t("recentTable.columns.missingFields")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-[#30363d]">
              {properties.map((prop) => (
                <tr key={prop.id} className="transition-colors duration-150 hover:bg-gray-50/40 dark:hover:bg-[#1c2128]">
                  ...
                  <td className="px-6 py-4 text-gray-600 dark:text-[#7d8590]">
                    {/* Use translatePropertyType helper — handles normalization + i18n lookup */}
                    {prop.propertyType ? translatePropertyType(t, prop.propertyType) : '—'}
                  </td>
                  <td className="px-6 py-4">
                    {prop.verified ? (
                  ...
                  <td className="px-6 py-4 text-gray-500 dark:text-[#7d8590]">
                    {prop.missingFields?.length > 0
                      ? t("recentTable.missingCount", { count: prop.missingFields.length })
                      : t("recentTable.noneMissing")}
                  </td>
                </tr>
              ))}
            </tbody>
