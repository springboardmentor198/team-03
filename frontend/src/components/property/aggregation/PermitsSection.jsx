...
export default function PermitsSection({ section }) {
  ...
  return (
    <SectionCard
      title={t("property.aggregation.permits.title")}
      subtitle={t("property.aggregation.permits.subtitle")}
      icon={FileCheck}
      section={section}
      emptyLabel={t("property.aggregation.permits.emptyLabel")}
    >
      {permits?.length > 0 ? (
        <ul className="space-y-3">
          {permits.map((p, i) => {
            const permitTypeLabel = p.permitType
              ? t(`property.aggregation.enums.permitType.${p.permitType}`, {
                  defaultValue: p.permitType,
                })
              : "";
            const permitStatusLabel = p.status
              ? t(`property.aggregation.enums.permitStatus.${p.status}`, {
              ...
              >
                ...
                {(p.issueDate || p.expiryDate) && (
                  <div className="mt-3 flex items-center gap-4 border-t border-gray-100 dark:border-[#30363d] pt-2 text-[11px] text-gray-600 dark:text-[#7d8590]">
                    {p.issueDate && (
                      <span>
                        <span className="font-semibold">{t("property.aggregation.permits.issued")}</span>{" "}
                        {formatDate(p.issueDate)}
                      </span>
                    )}
                    {p.expiryDate && (
                      <span>
                        <span className="font-semibold">{t("property.aggregation.permits.expires")}</span>{" "}
        ...
        <p className="text-sm text-gray-500 dark:text-[#7d8590] text-center py-4">
          {t("property.aggregation.permits.noRecords")}
        </p>
      ) : null}
    </SectionCard>
  );
