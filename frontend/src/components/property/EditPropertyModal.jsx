...
export default function EditPropertyModal({ isOpen, onClose, property, onSuccess }) {
  const [form, setForm] = useState({
    ...
    yearBuilt: "", bedrooms: "", bathrooms: "", stories: "", imageUrl: "",
  });
...
function Field({ label, icon: Icon, required, optional, error, hint, children }) {
  return (
    <div>
      ...
      <div className="relative">
        {Icon && (
          <Icon className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 dark:text-[#6e7681] z-10" />
        )}
        {children}
      </div>
      {error && (
...
const inputClsNoIcon = () =>
  `h-11 w-full rounded-xl border border-gray-200 dark:border-[#30363d] bg-white dark:bg-[#0d1117] px-3 text-sm text-gray-800 dark:text-[#e6edf3] placeholder:text-gray-400 dark:placeholder:text-[#6e7681] outline-none transition disabled:bg-gray-50 dark:disabled:bg-[#1c2128] focus:border-[#22C55E] focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/30 hover:border-gray-300 dark:hover:border-[#484f58]`;