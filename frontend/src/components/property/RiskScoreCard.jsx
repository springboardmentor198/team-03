...
// Category config uses translation keys instead of hardcoded labels
const CATEGORIES = [
  { key: "financialScore",     labelKey: "property.risk.categories.financial",     weight: 30 },
  { key: "legalScore",         labelKey: "property.risk.categories.legal",         weight: 30 },
  { key: "environmentalScore", labelKey: "property.risk.categories.environmental", weight: 25 },
  { key: "structuralScore",    labelKey: "property.risk.categories.structural",    weight: 15 },
];

// Risk label → translation key
const RISK_LABEL_KEY = {
  LOW: "property.card.lowRisk",
  MEDIUM: "property.card.mediumRisk",
  HIGH: "property.card.highRisk",
};
...
export default function RiskScoreCard({ propertyId }) {
  ...
  useEffect(() => {
    ...
    const fetchRisk = async () => {
      try {
        ...
        if (!cancelled) setError(err?.message || t("property.risk.couldntLoad"));
      } finally {
    ...
    return (
      <div className="rounded-2xl border border-gray-100 dark:border-[#30363d] bg-white dark:bg-[#161b22] p-6 shadow-sm">
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-[#7d8590]">
          ...
          <span>{t("property.risk.unavailable")}</span>
        </div>
      </div>
    );
  }
  ...
  const riskLabelKey = RISK_LABEL_KEY[risk.riskLabel] ?? RISK_LABEL_KEY.MEDIUM;
  ...
  return (
    <div className="rounded-2xl border border-gray-100 dark:border-[#30363d] bg-white dark:bg-[#161b22] shadow-sm overflow-hidden">
      ...
      <div className="p-6 space-y-4">
        <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-[#6e7681]">
          {t("property.risk.categoryBreakdown")}
        </p>

        {CATEGORIES.map(({ key, labelKey, weight }) => (
          <div key={key}>
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <span className="text-[10px] text-gray-400 dark:text-[#6e7681] font-medium">
                {t("property.risk.weight", { pct: `${weight}%` })}
              </span>
            </div>
        ...
        <>
          ...
          <div className="p-6 space-y-2">
            <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-[#6e7681] mb-3">
              {t("property.risk.factorsCount", { n: risk.riskFlags.length })}
            </p>
        ...
        <>
          ...
          <div className="flex items-start gap-2 px-6 py-4">
            ...
            <p className="text-[11px] text-gray-400 dark:text-[#7d8590] leading-snug">
              {t("property.risk.incompleteNote")}
            </p>
          </div>
        </>
      )}
