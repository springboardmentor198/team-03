...
const TOPICS = [
  { value: "general", label: "General enquiry", desc: "Product questions, feedback" },
  { value: "enterprise", label: "Enterprise sales", desc: "Custom deals, volume pricing" },
  { value: "technical", label: "Technical support", desc: "Bugs, integration help" },
  { value: "billing", label: "Billing", desc: "Invoices, refunds, plan changes" },
  { value: "partnership", label: "Partnership", desc: "Integrations, resellers" },
];

const TESTIMONIALS = [
  {
    quote:
      "We ran 47 property verifications through this platform before our last acquisition. The risk breakdown caught a tax-lien issue our lawyers missed.",
    name: "Priya Raghavan",
    role: "Head of Investments, Meridian Capital Advisors",
  },
  {
    quote:
      "Their team replied to our enterprise enquiry within 3 hours and had us onboarded with a custom integration in a week.",
    name: "Arjun Mehta",
...
export default function ContactPage() {
  ...
  const validate = () => {
    ...
    if (!form.message.trim() || form.message.trim().length < 10)
      next.message = "Tell us a bit more (min 10 characters)";
    setErrors(next);
      ...
      <div className="max-w-7xl mx-auto px-6 pt-32 pb-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-5 text-[15px] leading-relaxed text-white/50 max-w-xl">
            No ticket queues, no canned replies. Every message goes straight to our
            engineering and due-diligence team — we reply within 24 hours on business days.
          </p>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    />
                    {errors.email && (
                      <p className="mt-1.5 text-[12px] text-red-400">{errors.email}</p>
                    )}
                  </div>
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6 space-y-5">
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  <p className="text-[13px] font-semibold text-white">Remote-first team</p>
              ...
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  ...
                  <a
                    href="mailto:duedeligence8@gmail.com"
                    className="text-[13px] text-emerald-400 hover:text-emerald-300 transition mt-1 inline-block"
                  >
              ...
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-white/50 leading-relaxed mt-1">
                    ...
                    <br />
                    Enterprise customers: 4-hour SLA.
                  </p>
                </div>
              </div>
            </div>
...
/* ─────────────────────────────────────────────────────────
   Premium custom dropdown (shadcn-style)
   ───────────────────────────────────────────────────────── */
function PremiumTopicSelect({ value, onChange }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className={`w-full h-11 rounded-lg bg-white/[0.04] border px-4 text-sm text-white flex items-center justify-between transition ${
          open
      ...
      {open && (
        <div className="absolute z-30 mt-2 w-full rounded-xl border border-white/10 bg-[#0f0f0f] shadow-2xl shadow-black/60 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
          {TOPICS.map((t) => {
